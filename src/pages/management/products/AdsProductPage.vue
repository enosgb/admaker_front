    ...
    <div class="flex flex-col items-center justify-center gap-4">
      <div class="bg-white p-6 shadow-lg rounded-2xl">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <InputText v-model="form.stock" input-name="stock" class="text-gray-400 col-span-2" label="Estoque:"
            input="estoque" />
          <InputText v-model="form.cost_price" input-name="cost_price" class="text-gray-400 col-span-2" label="Preço:"
            input="cost_price" />
          <InputText v-model="form.sale_price" input-name="sale_price" class="text-gray-400 col-span-2"
            label="Preço de venda:" input="sale_price" />
          <InputSelect v-model="form.category" input-name="category" class="text-gray-400 col-span-2" label="Categoria:"
            input="category" :options="categories.map((x) => ({ label: x.name, value: String(x.id) }))" />
...
const { categories, fetchCategories } = useAdsCategories()
...
function handleCreateProduct(e: Event) {
  ...
  formData.append('category', String(form.category || 0));
